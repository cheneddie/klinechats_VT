...
async function loadCase(caseId){let c=state.cases.find(x=>x.id===caseId);if(c||!state.apiOnline)return c;try{c=normalizeCase(await api('/cases/'+encodeURIComponent(caseId)),state.cases.length);mergeCases([c]);return c}catch{return null}}
...
FabioV2.store={state,boot,api,recalc,casesForNode,loadNodeCases,loadCase,record,dueReviews,markReviewDone,weakestNodes,highConfidenceMistakes,exportHistory,updateSettings,persist};
})();
