...
async function fetchWindow(c,{nodeId=null,timeframe='1m',before=1,after=1,session='full'}={}){
  const q=new URLSearchParams({timeframe,days_before:String(before),days_after:String(after),session});
...
async function render(host,c,opts={}){
  ...
  dispose();
  host.innerHTML='<div class="empty-chart">V4 Replay：載入前後交易日資料…</div>';
  let payload;
  try{
    payload=await fetchWindow(c,opts);
  }catch(e){
  ...
  if(opts.hideFuture){
    const cut=Date.parse(metaTime(c,opts.nodeId));
    if(Number.isFinite(cut))bars=bars.filter(b=>Number(b.timestamp)<=cut);
  }
  if(!bars.length){
    host.innerHTML='<div class="empty-chart">此範圍沒有 Replay bars。</div>';
    return null;
  }
  host.innerHTML='';
...
FabioV4.chart={render,fetchWindow,dispose,current:()=>active,timeframes:[...fmt.seconds,...fmt.minutes]};
})();
