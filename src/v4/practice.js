...
const state={nodeId:null,queue:[],i:0,confidence:3,answered:false,loading:false,timeframe:'1m',token:0,error:null,waitingHydration:false};
...
function loading(nodeId,msg='從 SQLite /api/cases 直接抓此 Node，不再只依賴瀏覽器前 2,000 筆。'){
  const n=FabioV2.nodeMap?.[nodeId],content=$('#content');if(!content)return;
  content.innerHTML=`<div class="v4-practice-root v4-loading"><h2>${esc(n?.code||nodeId)} · 載入專項案例</h2><p>${esc(msg)}</p><div class="v4-spinner"></div></div>`;
}
function errorView(nodeId,error){
  const content=$('#content'),n=FabioV2.nodeMap?.[nodeId];if(!content)return;
...
async function start(nodeId,force=false){
  const token=++state.token;
  state.nodeId=nodeId||'MR_CLEAR_RECLAIM';state.loading=true;state.i=0;state.answered=false;state.error=null;
  loading(state.nodeId);
  const count=Math.max(1,Number(FabioV2.store.state.settings.questionCount||20));
  try{
    state.queue=await balancedQueue(state.nodeId,count);
    if(!state.queue.length&&!FabioV2.store.state.apiOnline&&!force){
      state.waitingHydration=true;
      throw new Error('等待 Local Data API hydration；連線完成後會自動重試。');
    }
    state.waitingHydration=false;
  }catch(e){
    state.queue=[];state.error=String(e?.message||e);
  }
  if(token!==state.token)return;
  state.loading=false;
  if(state.error&&!state.queue.length){errorView(state.nodeId,state.error);return}
  renderQuestion();
}
function explain(c,nodeId){
  try{
    ...
    const metricRows=Object.entries(metrics).filter(([,v])=>v!==null&&v!==undefined).slice(0,8).map(([k,v])=>`<li><span>${esc(k)}</span><b>${esc(typeof v==='number'?Number(v).toFixed(3):v)}</b></li>`).join('');
    return `<div class="v4-explain"><b>${esc(v.code)} · ${esc(v.label)}</b><p>${esc(m.reason_code||'')}</p><ul>${(v.reason||[]).map(x=>`<li>${esc(x)}</li>`).join('')}${metricRows}</ul></div>`;
  }catch{return''}
}
async function renderChart(c){
  const host=$('#v4PracticeChart');if(!host)return;
  const current=await FabioV4.chart.render(host,c,{
    nodeId:state.nodeId,timeframe:state.timeframe,before:1,after:state.answered?1:0,session:'full',
  ...
  const info=$('#v4PracticeInfo');
  if(info&&current?.payload){
    const p=current.payload;
    info.textContent=`${(p.dates||[]).join(' → ')} · ${p.source_rows||0} physical rows · ${p.timeframe||state.timeframe}${p.cutoff_time?' · 截止 '+p.cutoff_time:''}`;
  }
}
function answer(v){
...
function next(){if(state.i+1>=state.queue.length){finish();return}state.i++;state.answered=false;renderQuestion()}
function finish(){
  ...
  $('#v4Again').onclick=()=>start(state.nodeId,true);
}
function renderQuestion(){
  const content=$('#content');if(!content)return;
  const n=FabioV2.nodeMap?.[state.nodeId],c=state.queue[state.i];
  if(!c){errorView(state.nodeId,'此 Node 在目前 Event Store 沒有可練案例。請先完成 Scanner。');return}
  ...
  content.innerHTML=`<div class="v4-practice-root">
    <div class="v4-practice-top">
      ...
      <span>第 ${state.i+1} / ${state.queue.length} 題</span><span>${esc(c.date)} · ${esc(c.strategy)} · ${c.direction==='long'?'多':'空'}</span>
    ...
    <div id="v4PracticeInfo" class="v4-r-note"></div>
    <div class="v4-practice-layout">
      <section><div id="v4PracticeChart" class="chart"></div></section>
      <aside class="v4-question"><span>${esc(n?.code)} · ${esc(n?.stage)}</span><h2>${esc(n?.question)}</h2>
        ...
        <div id="v4PracticeFeedback"></div><div id="v4Explain"></div><button id="v4Next" class="primary wide" disabled>下一題 →</button>
      </aside>
    </div></div>`;
  $('#v4PracticeNode').onchange=e=>start(e.target.value,true);
  ...
  $('#v4Yes').onclick=()=>{answer(true);$('#v4Next').disabled=false};$('#v4No').onclick=()=>{answer(false);$('#v4Next').disabled=false};$('#v4Next').onclick=next;
  renderChart(c);
}
function ensure(){
