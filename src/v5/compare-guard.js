(()=>{
  ...
  function render(){
    ...
    if(!host)return;
    // Wait until the original Compare handler has rendered its result table.
    // Rendering before that creates a race where the legacy handler overwrites the gate.
    if(!host.querySelector('table'))return;
    if(host.dataset.compareGuardToken===latest.token)return;
